...
namespace WcfService
{
    public class ObservacionService : IObservacion
    {
        ...
        public List<Observacion> ListarLibrosPendientesAlumno(string codAlumno)
        {
            ...
            if (ObservacionDAO.ListarObservaciones(codAlumno).Count >= 5)
            {
                throw new WebFaultException<string>(
                        "El alumno tiene mas de cinco observaciones, por favor acercarse a la institución para una reunión con Bienestar Estudiantil", HttpStatusCode.InternalServerError);
            }
